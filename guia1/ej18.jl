...
# ╔═╡ 78d750a9-31e7-4bfc-ac0b-1cdffbb7c386
using Plots,DifferentialEquations

# ╔═╡ e1f8d094-57be-11ee-1149-a7249325b089
md""" # Ejercicio 18 """

# ╔═╡ 31b5a389-3271-4d58-bd3a-94212d225887
md""" # a) """

# ╔═╡ 9426331a-c551-4b7e-a339-c6be8b926a64
function mov_bala!(du,u,p,t) # u = velocidad, posicion
	g = p[1]
	m = p[2]
	du[1] = g
	du[2] = -u[1]
...
# ╔═╡ 73bfc8ff-de5f-4145-bcec-ac5c3e336ffa
function resolucion_mov(i,cb) 
	# i = {1,2} --> 1 = bala cañon, 2 = bala mosquete
	# cb = callback
	...
	P₁ = ODEProblem(mov_bala!,u₀,t,p₁)
	P₂ = ODEProblem(mov_bala!,u₀,t,p₂)
	P = [P₁,P₂]
	solve(P[i],callback=cb)
end

# ╔═╡ 35a06ca6-1c93-4d39-92c1-4a2358f8233e
function condicion_rebote(u,t,integrator)
	u[2] <= 0
end

# ╔═╡ 6d11ff04-ac9d-4076-a119-4eab22f98c64
function rebote!(integrator)
	integrator.u[1] = -integrator.u[1]
	integrator.u[2] = -integrator.u[2]
end

# ╔═╡ e8510dff-5089-4b75-bf81-1e1e983159ce
dc = DiscreteCallback(condicion_rebote,rebote!)

# ╔═╡ 9a48c099-a819-411b-bed1-23b560450f2b
begin
	s₁ = resolucion_mov(1,dc)
	ts = 0:0.01:35
	plot(ts,s₁(ts)[2,:],label= "y(t) bala de cañon" )
end

# ╔═╡ 2e18e336-ef3e-4b35-9523-6d8cd9fe604b
md""" # b) """

# ╔═╡ f16654a6-aa1f-464d-b179-3c7d6752c072
function cont_condicion_rebote(u,t,integrator)
	u[2]
end

# ╔═╡ 8bea63fa-1c15-4c44-bff4-8859920d8738
function rebote_cont!(integrator)
	integrator.u[1] = -integrator.u[1]
end

# ╔═╡ 462041b2-d2b8-4c14-b89f-e5bbe9e08e00
cc = ContinuousCallback(cont_condicion_rebote,rebote_cont!)

# ╔═╡ fb2bb471-4fbf-4bbf-86c2-14b32e4669ff
begin
	s₂ = resolucion_mov(1,cc)
	ts₂ = 0:0.01:35
	plot(ts₂,s₂(ts₂)[2,:],label= "y(t) bala de cañon" )
end

# ╔═╡ a3e0c2e6-5691-413d-a025-76990a466f0d
md""" # c) """

# ╔═╡ 20d90bd4-9905-4e4a-a32b-31c96c771ed3
function mov_bala_roz!(du,u,p,t) # u = velocidad, posicion
	g = p[1]
	m = p[2]
	cᵣ = p[3]
	du[1] = g - cᵣ*(u[1]^2)/m
...
# ╔═╡ 1e0622a2-e81b-445a-9994-1ab34fe1659f
function resolucion_mov_roz(i,cb) 
	# i = {1,2} --> 1 = bala cañon, 2 = bala mosquete
	# cb = callback
	...
	P₁ = ODEProblem(mov_bala_roz!,u₀,t,p₁)
	P₂ = ODEProblem(mov_bala_roz!,u₀,t,p₂)
	P = [P₁,P₂]
	solve(P[i],callback=cb)
end

# ╔═╡ 53a40544-2370-4f1b-bdfb-4cad49af1aaf
begin
	s₃ = resolucion_mov_roz(1,cc)
	ts₃ = 0:0.01:35
	plot(ts₃,s₃(ts₃)[2,:],label= "y(t) bala de cañon" )
end
