...
# ╔═╡ 1c8583fa-276f-49a9-a371-38c9a6a7fd6a
using Plots
...
# ╔═╡ 3075be79-4790-428f-856d-47402d51bea6
function euler(f, t0, tf, u0, h)
	
	t = t0:h:tf # Desde t0 hasta tf con paso h
	n = length(t)
	u = zeros(n)
	u[1] = u0
...
# ╔═╡ ef1b6b40-f84d-4786-b39e-75a3d6e480e0
begin
	f(t,u) = 1 / u
	T,U = euler(f, 1, 5, 1, 0.1)
end

# ╔═╡ 349bdd7a-2ad7-43ee-a03d-27b8946a6ebf
plot(T,U)

# ╔═╡ 7e28d11e-f408-4ca8-b75b-777853f23b86
md""" # Ejercicio 1"""

# ╔═╡ 557ce3ba-ed6a-4d0c-8f33-454e4ebb18ba
begin
	λ = 1
	g(t,u) = λ*u
	T2, U2 = euler(g, 0, 5, 1, 0.01)
end

# ╔═╡ e484a06c-2dfa-4dfb-a9b3-704ad01552e8
plot(T2,[U2, exp.(T2*λ)], label = ["aprox" "real"])

# ╔═╡ 3e56b079-1158-4873-b503-d9dc5c5416b1
md""" ##### La solucion exacta es la exponencial"""

# ╔═╡ 42e7c0f7-0559-473e-a827-ba4b4758d2fe
md""" ##### Resultados malos"""

# ╔═╡ 5a8a8258-0c6f-4c5b-8993-32868edf30fb
begin
	λ2 = 4
	g2(t,u) = λ2*u
	T3, U3 = euler(g2, 0, 5, 1, 0.1)
end

# ╔═╡ 34e62d91-57b3-4b92-98f3-ee06ab37d28a
plot(T3,[U3, exp.(T3*λ2)], label = ["aprox" "real"])

# ╔═╡ 132f5c0f-ed29-47ce-85d3-4c4c1e0f8a03
function ejercicio1()
	λ = 3
	H = 2. .^collect(-2:-1:-8) # Elijo h pequeños, cada uno la mitad del anterior
	f(t,u) = λ*u
	tt = 0:0.001:5
	fig = plot(tt, exp.(λ*tt), label = "exacta", legend =:top)
	for h in H
		T,U = euler(f, tt[1], last(tt), 1, h)
		plot!(fig, T, U, label = "$h")
	end
	return fig
end

# ╔═╡ 71d2d518-8e1f-4c52-b747-960b8f1971e3
ejercicio1()
...
function ejercicio2()
	...
	fig = plot()
	for h in H
		T,U = euler(f, tt[1], last(tt), 0.15, h)
		plot!(fig, T, U, label = "$h")
	end
	return fig
end
...
# ╔═╡ a6631490-9c46-4fdf-a698-08e830b1d2c2
function euler_vec(f, t0, tf, u0, h)
	
	t = t0:h:tf # Desde t0 hasta tf con paso h
	n = length(t)
	k = length(u0) # Cantidad de variables
	u = zeros(k,n)
	u[1] = u0
...
# ╔═╡ c70d77d5-6191-4b20-87c1-3ef1e85ef7bc
md""" Con esto se puede terminar hasta Runge-Kutta"""
...
julia_version = "1.9.2"
manifest_format = "2.0"
project_hash = "f5e0992b8f7c9ae9e90d71b75f0d635c4de6b497"
...
git-tree-sha1 = "ca23a3d2843003f08c6e38a18f16144db4093a4e"
