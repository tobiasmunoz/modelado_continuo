...
# ╔═╡ 5e8022a6-ea24-4b66-9bd5-108d0f0e4435
using DifferentialEquations, Plots

# ╔═╡ 301970dc-4743-11ee-2af9-65d6cddd3efa
md""" # Ejercicio 12 """

# ╔═╡ ec8663cc-f712-4927-b427-efc6e80cf744
function tres_cuerpos!(du,u,p,t)
	
	G, m₁, m₂, m₃ = p
	
	du[1,1] = u[1,2]
	du[1,2] = ( G*m2*(u[2,1] - u[1,1])/( (u[2,1] - u[1,1])^2 + (u[2,3] - u[1,3])^2 )^(3/2) ) + ( G*m3*(u[3,1] - u[1,1])/( (u[3,1] - u[1,1])^2 + (u[3,3] - u[1,3])^2 )^(3/2) )
	du[1,3] = u[1,4]
	du[1,4] = ( G*m2*(u[2,3] - u[1,3])/ ( (u[2,1] - u[1,1])^2 + (u[2,3] - u[1,3])^2 )^(3/2) ) + ( G*m3*(u[3,3] - u[1,3])/( (u[3,1] - u[1,1])^2 + (u[3,3] - u[1,3])^2 )^(3/2) )

	du[2,1] = u[2,2]
	du[2,2] = ( G*m1*(u[1,1] - u[2,1])/( (u[2,1] - u[1,1])^2 + (u[2,3] - u[1,3])^2 )^(3/2) ) + ( G*m3*(u[3,1] - u[2,1])/( (u[3,1] - u[2,1])^2 + (u[3,3] - u[2,3])^2 )^(3/2) )
	du[2,3] = u[2,4]
	du[2,4] = ( G*m1*(u[1,3] - u[1,3])/ ( (u[2,1] - u[1,1])^2 + (u[1,3] - u[2,3])^2 )^(3/2) ) + ( G*m3*(u[3,3] - u[1,3])/( (u[3,1] - u[1,1])^2 + (u[3,3] - u[1,3])^2 )^(3/2) )
	
	du[3,1] = u[3,2]
	du[3,2] = ( G*m1*(u[1,1] - u[3,1])/( (u[1,1] - u[3,1])^2 + (u[3,3] - u[1,3])^2 )^(3/2) ) + ( G*m2*(u[2,1] - u[3,1])/( (u[2,1] - u[3,1])^2 + (u[3,3] - u[2,3])^2 )^(3/2) )
	du[3,3] = u[3,4]
	du[3,4] = ( G*m1*(u[1,3] - u[3,3])/( (u[1,1] - u[3,1])^2 + (u[3,3] - u[1,3])^2 )^(3/2) ) + ( G*m2*(u[2,3] - u[3,3])/( (u[2,1] - u[3,1])^2 + (u[3,3] - u[2,3])^2 )^(3/2) )
...
tres_cuerpos_prob = ODEProblem(tres_cuerpos!, U₀, ts, masas )

# ╔═╡ d3bd30d8-4160-455e-bf3a-7b58d850e4c9
sol_tres_cuerpos = solve(tres_cuerpos_prob)
