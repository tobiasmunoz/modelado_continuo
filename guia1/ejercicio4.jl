...
# ╔═╡ cab1458f-093b-487d-9edf-ce00df8b06b8
using Plots

# ╔═╡ 6780e21a-4ccb-45ab-9677-d60ba7a65291
md""" # Ejercicio 4 """

# ╔═╡ dc0ea9a6-403f-11ee-156f-499fc7f08128
begin
	
	r(t) = 0.2 + 0.2*cos(2*π*t)
	K = 1000
	m = 0.1
...
# ╔═╡ eef0743b-8d07-45a9-9d3d-fab96ef64a8b
function heun(f, t0, tf, y0, h)
	...
	for i in 1:n-1
		y[i+1] = y[i] + (h/4)*( f(t[i], y[i] ) + 3*f( t[i] + 2*h/3, y[i] + 2*h*f(t[i],y[i])/3 ) )
	end
	return t,y
end

# ╔═╡ 83fd6d71-3251-4dda-bfa8-98cc7e6cd233
md""" # b) , c) """

# ╔═╡ fb60f587-9382-4d1f-a0a3-a291f315ae8a
begin
	
	# El tiempo es medido en años
	# Estimacion de la evolucion de la poblacion a lo largo de 50 años
	
	h = 1/365
	Y0 = [500,1000]
	T_100, Y_100 = heun(f, 0, 50, 100, h)
	fig = plot(T_100, Y_100, label = "yₒ = 100")

	for y0 in Y0
		T_temp, Y_temp = heun(f, 0, 50, y0, h)
		plot!(T_temp, Y_temp, label = "yₒ = $y0")
	end
