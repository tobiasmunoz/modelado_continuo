...
# ╔═╡ f7cbaa7e-4cb8-11ee-2d09-4bf247759fc8
using DifferentialEquations,DiffEqParamEstim,Plots, CSV, DataFrames,Optimization, OptimizationOptimJL

# ╔═╡ 322cfa28-19b9-479a-8611-6c16a09a2c58
md""" # Ejercicio 13 """

# ╔═╡ 85bcefef-cb01-4d6a-8fac-758decccf4be
df = CSV.read("pop_world.csv",DataFrame;header=4)

# ╔═╡ ca895cea-d3d9-479f-90ee-2a2ffda1cf54
begin paises = ["Angola","China","Nigeria"]
	
sel = []
	
for p in paises
	push!( sel,filter(row->row["Country Name"]==p,df) )
end
	
end

# ╔═╡ e11463eb-1945-4497-acc0-b237f3ead224
begin
	
selv = []
	
for i in 1:3
	push!( selv,collect(values(sel[i][1,5:end])) )
end
	
end

# ╔═╡ a19bfbd3-fa3f-4484-bf04-ba00456e8dea
md""" # a) """

# ╔═╡ be7c4271-f5c1-4afa-91b1-893565a7a632
begin
	figs = []
	i = 1
	for p in selv
		push!(figs,scatter(1960:2022,p,title=sel[i][1,1]))
		i = i + 1
	end

	plot(figs[1], figs[2], figs[3], layout = (3,1))
	
end

# ╔═╡ c480c4e6-ced6-472b-a896-b98d7958bc7d
md""" # b) """

# ╔═╡ 1c574dcc-1423-44f2-bdc4-7ec86576a74f
mod_exp(x,μ,t) = μ[1]*x 

# ╔═╡ 72865539-596c-43d0-94f8-9b2a9bdb7930
begin
	x0₁ = selv[1][1]
	x0₂ = selv[2][1]
	x0₃ = selv[3][1]
...
# ╔═╡ 4d6e52e6-6a29-471b-b188-243d67763de3
begin
	t = 1960:2022
	tspan = (t[1],t[end])
end

# ╔═╡ 08611c1b-a545-446c-8c70-5f064c200b6e
begin
	prob_exp₁ = ODEProblem(mod_exp,x0₁,tspan,[0.1])
	prob_exp₂ = ODEProblem(mod_exp,x0₂,tspan,[0.1])
	prob_exp₃ = ODEProblem(mod_exp,x0₃,tspan,[0.1])
end

# ╔═╡ 659ab248-1b47-43eb-9c1f-0248c6312b94
begin
	sol_exp₁ = solve(prob_exp₁)
	plot(sol_exp₁, title = "Angola")
	scatter!(t,selv[1])
end

# ╔═╡ b66dc543-1a31-419e-ad14-ee3a9ba9c1fb
begin
	sol_exp₂ = solve(prob_exp₂)
	plot(sol_exp₂, title = "China")
	scatter!(t,selv[2])
end

# ╔═╡ 319256ea-7365-4112-9a6e-2613ed693165
begin
	sol_exp₃ = solve(prob_exp₃)
	plot(sol_exp₃, title = "Nigeria")
	scatter!(t,selv[3])
end

# ╔═╡ c1aebdac-d5b2-4a9a-9c81-0649f2d399b5
begin
	func_exp₁ = build_loss_objective(prob_exp₁,Tsit5(),L2Loss(t,selv[1]),Optimization.AutoFiniteDiff(),maxiters=10000,verbose=false)
	
	func_exp₂ = build_loss_objective(prob_exp₂,Tsit5(),L2Loss(t,selv[2]),Optimization.AutoFiniteDiff(),maxiters=10000,verbose=false)
	
	func_exp₃ = build_loss_objective(prob_exp₃,Tsit5(),L2Loss(t,selv[3]),Optimization.AutoFiniteDiff(),maxiters=10000,verbose=false)
end

# ╔═╡ 659f2463-f04c-48f1-8cac-407b1cf59e77
optprob_exp₁ = OptimizationProblem(func_exp₁,[0.1],lb=[0.],ub=[1.])

# ╔═╡ 0e099acc-8e03-4c12-bdc7-18394ba83548
plot(0:0.001:0.025,func_exp₁)

# ╔═╡ 42b11776-e8da-46ba-a982-49114053c630
μ̄_exp₁ = solve(optprob_exp₁,BFGS())

# ╔═╡ b7f8a1be-5715-4bfc-8d73-4f28f39fbf32
begin
	prob_exp₁2 = remake(prob_exp₁,p=μ̄_exp₁)
	sol_exp₁2  = solve(prob_exp₁2)
	plot(sol_exp₁2, title = "Angola")
	scatter!(t,selv[1])
end

# ╔═╡ 282533dd-2a1d-4514-9932-94d808086234
begin
	optprob_exp₂ = OptimizationProblem(func_exp₂,[0.1],lb=[0.],ub=[1.])
	μ̄_exp₂ = solve(optprob_exp₂,BFGS())
	...
	prob_exp₂2 = remake(prob_exp₂,p=μ̄_exp₂)
	sol_exp₂2  = solve(prob_exp₂2)
	plot(sol_exp₂2, title = "China")
	scatter!(t,selv[2])
end

# ╔═╡ c35d0cb0-5b53-418f-80fb-7b9d6cda9d4c
begin
	optprob_exp₃ = OptimizationProblem(func_exp₃,[0.1],lb=[0.],ub=[1.])
	μ̄_exp₃ = solve(optprob_exp₃,BFGS())
	...
	prob_exp₃2 = remake(prob_exp₃,p=μ̄_exp₃)
	sol_exp₃2  = solve(prob_exp₃2)
	plot(sol_exp₃2, title = "Nigeria")
	scatter!(t,selv[3])
end

# ╔═╡ 0605c088-5b50-429e-ab06-93a711ff6ef1
md""" # c) 
Resuelvo solo para Angola"""

# ╔═╡ fe8ecb6c-b52b-44e5-95ff-b78455a47592
mod_log(x,μ,t) = μ[1]*x*(1-x/μ[2])

# ╔═╡ f6698a51-c026-46fe-ab23-a87b8ebe30ff
prob_log₁ = ODEProblem(mod_log,x0₃,tspan,[0.1,45_000_000])

# ╔═╡ d05864bd-f0cd-4eb0-8dd0-a417343966cc
func_log₁ = build_loss_objective(prob_log₁,Tsit5(),L2Loss(t,selv[3]),  			                      Optimization.AutoFiniteDiff(),maxiters=10000,verbose=false)

# ╔═╡ ab812eef-f1dc-4fba-b42c-7bc39b99f841
optprob_log₁ = OptimizationProblem(func_log₁,[0.1,45_000_000],lb=[0.,40_000_000],ub=[1.,300_000_000])

# ╔═╡ ea2e94d2-74c5-41ec-bcce-f20500f4126e
μ̄_log₁ = solve(optprob_log₁,BFGS())

# ╔═╡ 7d10318a-98cf-4757-af42-ca32b25afaac
begin
	prob_log₁2 = remake(prob_log₁,p=μ̄_log₁)
	sol_log₁2  = solve(prob_log₁2)
	plot(sol_log₁2, title = "Nigeria")
	scatter!(t,selv[3])
end
...
[[deps.Optimization]]
deps = ["ADTypes", "ArrayInterface", "ConsoleProgressMonitor", "DocStringExtensions", "LinearAlgebra", "Logging", "LoggingExtras", "Pkg", "Printf", "ProgressLogging", "Reexport", "Requires", "SciMLBase", "SparseArrays", "Symbolics", "TerminalLoggers"]
git-tree-sha1 = "8aea10e024a6ac845bf71818480ab8ac813d9caa"
