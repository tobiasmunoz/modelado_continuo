...
# ╔═╡ 9f1bec30-5236-11ee-0b32-77f583e35432
using DifferentialEquations,DiffEqParamEstim,Plots, CSV, DataFrames,Optimization, OptimizationOptimJL

# ╔═╡ a4d1e810-6692-4474-a381-067a705cb3c0
md""" # Ejercicio 15 """

# ╔═╡ 465babc2-1320-4e7e-8e7f-bd9ae380da8d
df = CSV.read("lynxhare.csv", DataFrame)

# ╔═╡ d88e38b6-294e-40b9-b818-c1c27e565974
begin
	t = df[1:end,1]
	lynx = df[1:end,3]
	hare = df[1:end,2]
	scatter(t, hare, label = "hare")
	scatter!(t, lynx, label = "lynx")
end

# ╔═╡ 28058592-9ce6-4b1f-b4de-df5f1d8fa10f
function predpresa(du,u,p,t)
	du[1] = -p[1]*u[1]+p[2]*u[1]*u[2]
	du[2] = p[3]*u[2]-p[4]*u[1]*u[2]
end

# ╔═╡ ab159110-d253-4b45-bc1e-50b732e8ee74
begin
	μ₀ = [df[1,2], df[1,3]]
	tspan = (df[1,1],df[end,1])
	prob = ODEProblem(predpresa, μ₀, tspan, abstol=1e-14,reltol=1e-10)
end

# ╔═╡ e5d4cb35-311f-4df4-82b0-df37348bed61
begin
	func    = build_loss_objective(prob,AutoTsit5(Rosenbrock23()),L2Loss(tspan,df),Optimization.AutoFiniteDiff())
	optprob = OptimizationProblem(func,0.1ones(4),lb=zeros(4),ub=0.5ones(4))
	p̄       = solve(optprob,SAMIN(),maxiters=50000)
end

# ╔═╡ c570abf6-4605-472f-919c-37b89d82ba6a
begin
	optsol  = solve(remake(prob ,u0=μ₀ , p=p̄))
	plot(optsol)
	scatter!(t, lynx, label = "lynx")
	scatter!(t, hare, label = "hare")
end

# ╔═╡ 7d0b2b76-6816-4660-9f5d-cb4b3eee75ec
begin
	hare32 = hare[end-31:end]
	lynx32 = lynx[end-31:end]

	hare30_standardized = []
	lynx30_standardized = []

	for i in 2:31
		push!( hare30_standardized, (hare32[i-1] + hare32[i] + hare32[i+1])/3 )
		push!( lynx30_standardized, (lynx32[i-1] + lynx32[i] + lynx32[i+1])/3 )
	end
end

# ╔═╡ a26dcea8-62bd-4589-a7e6-33d3aee4a3a1
begin
	t1 = 0:29
	scatter(t1, hare30_standardized, label = "hare")
	scatter!(t1, lynx30_standardized, label = "lynx")
end

# ╔═╡ 16758d05-922b-44c9-ab2b-ab9fd39e2393
begin
	u0 = [hare30_standardized[1], lynx30_standardized[1]]
	tspan2 = (0,29)
	prob2 = ODEProblem(predpresa, u0, tspan2, abstol=1e-14,reltol=1e-10)
end

# ╔═╡ 88d45fe2-b215-434f-a9c6-8a0e404c8df7
datos_suavizados = hcat(hare30_standardized,lynx30_standardized)

# ╔═╡ a3b0f679-fdd2-4928-9c43-80744ec40d66
begin
	func2   = build_loss_objective(prob2,AutoTsit5(Rosenbrock23()),L2Loss(tspan2,datos_suavizados),Optimization.AutoFiniteDiff())
	optprob2 = OptimizationProblem(func2,0.1rand(4),lb=zeros(4),ub=ones(4))
	p̄2       = solve(optprob2,SAMIN(),maxiters=100000)
end

# ╔═╡ 7c79d024-490b-47db-9416-4a8e61c90323
begin
	optsol2  = solve(remake(prob2 ,u0=u0 , p=p̄2))
	plot(optsol2)
	scatter!(0:29, lynx30_standardized, label = "lynx")
	scatter!(0:29, hare30_standardized, label = "hare")
end
...
git-tree-sha1 = "8489ad2e45328a9eba30a3c46006b04deee65a03"
