...
# ╔═╡ ebdb373e-09e3-44fb-a610-0d6dd2e7785c
using DifferentialEquations,DiffEqParamEstim,Plots, CSV, DataFrames,Optimization, OptimizationOptimJL

# ╔═╡ edd4c082-5246-11ee-229d-710e9bd747e6
md""" # Ejercicio 17 """

# ╔═╡ 35bdd778-2818-4b5e-8fa6-928538ca33d7
md""" p[1] = N, p[2] = β, p[3] = σ, p[4] = γ
	u[1] = S, u[2] = I, u[3] = R"""

# ╔═╡ be29d009-8b40-4041-942e-1850ffac870c
...
# ╔═╡ 2eee654d-3bc6-4a11-b98e-8afcbff40ed4
df = CSV.read("influenza_2022_2023.csv",DataFrame)

# ╔═╡ 35b4bbe9-b5a1-4285-a956-90594e57aadc
select!(df, Not([:"Column8"]))

# ╔═╡ 8d4ce754-f03a-43d8-b98c-5666544c9ed1
begin
	Ia = df[:,2]
	Ib = df[:,3]
	Sa = df[:,6] - Ia - Ib
...
# ╔═╡ 77fc969d-d3db-419d-b6c7-4913d1109510
datosA = hcat(Sa, Ia)

# ╔═╡ 6e26523b-d98d-461a-b5c2-dee25078effb
function loss(sol)
	if any((s.retcode != :Success for s in sol))
		return Inf
	else
		return sum((sol.(t,idxs=2)-data).^2) #+ 1000min(0,sol[coord_data+1,1])^2
	end
end

# ╔═╡ 131e93e8-2a40-466f-b4bf-15f5cd2c0fb5
begin
	
	tt = 1:42
	tspan = (tt[1],tt[end])
	u0 = [datosA[1,1], datosA[1,2]]

end

# ╔═╡ 0765a04f-77ca-4028-9298-953b69e9b759
begin
	
	prob = ODEProblem(sir, u0, tspan)
	func    = build_loss_objective(prob,AutoTsit5(Rosenbrock23()),loss,Optimization.AutoFiniteDiff())	
	optprob = OptimizationProblem(func,0.1rand(3),lb=zeros(3),ub=ones(3))
	p̄ = solve(optprob,SAMIN(),maxiters=100)
	
end

# ╔═╡ 064dc3d3-9911-4e32-b650-62fe5f35e86a
begin
	optsol  = solve(remake(prob,u0=u0,p=p̄))
	plot(optsol)
end
